...
uses
  ...
  ndsUnit in 'ndsUnit.pas' {ndsReportForm},
...
begin
  StrPCopy(menu_name,'Отчет по НДС');
end;

//----------------------------------------------------------------------
procedure InitDLL(Var Conf : _config); export;
begin
...
var
  ndsReportForm: TndsReportForm;
begin
  ndsReportForm := nil;
  if InitFlag then begin
    try
      ndsReportForm := TndsReportForm.Create(parentOwner);
      ndsReportForm.PathToProgram := parentConfig.PathToProgram;

      if (i1page and param) = 0 then
        ndsReportForm.InvPageControl.Pages[0].TabVisible := false;

      if (i2page and param) = 0 then
        ndsReportForm.InvPageControl.Pages[1].TabVisible := false;

      ndsReportForm.ShowModal;
    finally
      ndsReportForm.Free;
    end;
  end
