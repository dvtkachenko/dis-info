...
uses
  ...
  invoicesUnit in 'invoicesUnit.pas' {makInvExportForm},
...
var
  makInvExportForm: TmakInvExportForm;
begin
  makInvExportForm := nil;
  if InitFlag then begin
    try
      makInvExportForm := TmakInvExportForm.Create(parentOwner);
      makInvExportForm.PathToProgram := parentConfig.PathToProgram;

      if (i1page and param) = 0 then
        makInvExportForm.InvPageControl.Pages[0].TabVisible := false;

      makInvExportForm.ShowModal;
    finally
      makInvExportForm.Free;
    end;
  end
  else
